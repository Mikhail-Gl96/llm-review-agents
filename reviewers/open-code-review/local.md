...
Ставится один раз — работает из любого репо:

```bash
# Из этого репо:
./reviewers/open-code-review/install.sh

# Или одной командой (curl):
curl -sSL https://raw.githubusercontent.com/Mikhail-Gl96/llm-review-agents/main/reviewers/open-code-review/install.sh | bash
```

Скрипт установит Node (если нет), `@alibaba-group/open-code-review`, враппер и
конвертер в `/usr/local/`. После этого `ocr-review` доступен везде:

```bash
ocr-review                           # консоль, текущие изменения
ocr-review --markdown                # красивый markdown-отчёт
ocr-review --json                    # сырой JSON
ocr-review --from main --to HEAD     # диапазон коммитов
ocr-review --preview                 # показать, что попадёт в ревью
```

Если `/usr/local/bin` не в PATH — инсталлер подскажет, что дописать в `~/.zshrc`.

## 2. Установка вручную (без враппера)

Если глобальный враппер не нужен, можно поставить только CLI:
...
## 3. Конфигурация провайдера (один раз)
...
## 5. Свои правила ревью (необязательно)
...
## 6. Красивый markdown-вывод (JSON + конвертер)
